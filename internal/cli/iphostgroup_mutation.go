...
func newIPHostGroupCreateCmd(d RootDeps, cat *catalog.Catalog) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := iphostGroupSvc(d, cat).Create(cmd.Context(), profile, name, body, !yes)
			if err != nil {
				return err
			}
...
func newIPHostGroupUpdateCmd(d RootDeps, cat *catalog.Catalog) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := iphostGroupSvc(d, cat).Update(cmd.Context(), profile, name, body, expectedHash, ignoreHash, !yes)
			if err != nil {
				return err
			}
			return printObjectMutation(cmd, result)
...
func newIPHostGroupDeleteCmd(d RootDeps, cat *catalog.Catalog) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := iphostGroupSvc(d, cat).Delete(cmd.Context(), profile, name, expectedHash, ignoreHash, !yes)
			if err != nil {
				return err
			}
			return printObjectMutation(cmd, result)
