...
func TestServer_BootsAndListsZeroTools(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, result.Tools, 17, "T4 registers 4 auth tools, T5 registers 4 object tools, T6 registers 1 raw tool, T7 registers 4 host_ip tools, T8 registers 4 service tools")
}
