...
func (s *Server) handleMACHostCreate(ctx context.Context, _ *sdkmcp.CallToolRequest, in MACHostCreateInput) (*sdkmcp.CallToolResult, any, error) {
	...
	if len(profiles) == 1 {
		result, err := s.macHostSvc().Create(ctx, profiles[0], in.Name, in.Body, in.DryRun)
		if err != nil {
	...
	op := func(ctx context.Context, profile string, preflight bool) (any, error) {
		return s.macHostSvc().Create(ctx, profile, in.Name, in.Body, preflight || in.DryRun)
	}
	fr := svc.Run(ctx, "mac_host_create", profiles, op, in.DryRun)
...
func (s *Server) handleMACHostUpdate(ctx context.Context, _ *sdkmcp.CallToolRequest, in MACHostUpdateInput) (*sdkmcp.CallToolResult, any, error) {
	...
	if len(profiles) == 1 {
		result, err := s.macHostSvc().Update(ctx, profiles[0], in.Name, in.Body, in.ExpectedDiffHash, in.IgnoreExpectedDiffHash, in.DryRun)
		if err != nil {
			return s.errorEnvelopeResult(err, profiles[0])
		}
		return s.renderObjectMutation(result, profiles[0])
	...
	op := func(ctx context.Context, profile string, preflight bool) (any, error) {
		return s.macHostSvc().Update(ctx, profile, in.Name, in.Body, in.ExpectedDiffHash, in.IgnoreExpectedDiffHash, preflight || in.DryRun)
	}
	fr := svc.Run(ctx, "mac_host_update", profiles, op, in.DryRun)
	return s.renderFanoutResult(fr)
}
...
func (s *Server) handleMACHostDelete(ctx context.Context, _ *sdkmcp.CallToolRequest, in MACHostDeleteInput) (*sdkmcp.CallToolResult, any, error) {
	...
	if len(profiles) == 1 {
		result, err := s.macHostSvc().Delete(ctx, profiles[0], in.Name, in.ExpectedDiffHash, in.IgnoreExpectedDiffHash, in.DryRun)
		if err != nil {
			return s.errorEnvelopeResult(err, profiles[0])
		}
		return s.renderObjectMutation(result, profiles[0])
	...
	op := func(ctx context.Context, profile string, preflight bool) (any, error) {
		return s.macHostSvc().Delete(ctx, profile, in.Name, in.ExpectedDiffHash, in.IgnoreExpectedDiffHash, preflight || in.DryRun)
	}
	fr := svc.Run(ctx, "mac_host_delete", profiles, op, in.DryRun)
	return s.renderFanoutResult(fr)
}
