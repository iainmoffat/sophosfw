// Package svc — VPNProfile mutating surface (Phase 15, Task 5).
//
...
// VPNProfileSvc owns the create/update/delete surface for VPNProfile.
...
func (s *VPNProfileSvc) now() time.Time {
	if s.Now != nil {
		return s.Now()
	}
	return time.Now().UTC()
...
// requiredVPNProfileFields are the keys a create/update body must
...
// VPNProfile fields (encryption algorithm, hash, DH group, key lifetime
...
// Create adds a new VPNProfile. The body must include all keys listed
// in requiredVPNProfileFields. dryRun=true returns a Preview without
// sending the envelope.
func (s *VPNProfileSvc) Create(ctx context.Context, profileName, name string, body map[string]any, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "create", "", false, dryRun)
}

// Update replaces an existing VPNProfile body. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *VPNProfileSvc) Update(ctx context.Context, profileName, name string, body map[string]any, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "update", expectedHash, ignoreHash, dryRun)
}

// Delete removes an VPNProfile by name. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *VPNProfileSvc) Delete(ctx context.Context, profileName, name, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
...
func (s *VPNProfileSvc) mutate(ctx context.Context, profileName, name string, body map[string]any, op, expectedHash string, ignoreHash, dryRun bool) (out *ObjectMutationResult, err error) {
	...
	entryAudit := AuditEntry{
		...
		Operation:  "vpn_profile_" + op,
		ObjectType: "VPNProfile",
		ObjectName: name,
	}
	if expectedHash != "" {
	...
	catEntry, ok := s.Inner.Catalog.Resolve("VPNProfile")
	if !ok || !catEntry.Mutable {
		return nil, fmt.Errorf("%w: VPNProfile is not flagged mutable in the catalog", sophos.ErrInvalidRequest)
	}
	...
	if op != "delete" {
		for _, k := range requiredVPNProfileFields {
			v, present := body[k]
			if !present {
	...
	switch op {
	case "create":
		inner, merr := marshalObjectBody("VPNProfile", body)
		if merr != nil {
			return nil, merr
		}
	...
	case "delete":
		...
		inner.WriteString("<VPNProfile><Name>")
		if perr = xml.EscapeText(&inner, []byte(name)); perr != nil {
			return nil, perr
		}
		inner.WriteString("</Name></VPNProfile>")
...
// fetchLive loads the live VPNProfile body as a map[string]any. The
...
func (s *VPNProfileSvc) fetchLive(ctx context.Context, profileName, name string) (map[string]any, error) {
	obj, err := s.Inner.Get(ctx, profileName, "VPNProfile", name)
	if err != nil {
		return nil, err
	}
	m, ok := obj.Data.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("VPNProfileSvc: catalog returned non-map VPNProfile payload: %T", obj.Data)
	}
	if liveName, _ := m["Name"].(string); liveName == "" {
		...
		return nil, fmt.Errorf("VPNProfile %q: %w", name, sophos.ErrNotFound)
	}
	return m, nil
}
