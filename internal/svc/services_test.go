...
// TestServices_Create_DoesNotMutateCallerBody is a regression guard
...
func TestServices_Create_DoesNotMutateCallerBody(t *testing.T) {
	s, _, _ := newServicesSvc(t, nil)
	body := map[string]any{
		"Name": "ssh",
		"Type": "TCPorUDP",
		"ServiceDetails": map[string]any{
	...
	_, err := s.Create(context.Background(), "home", "ssh", body, true /* dryRun */)
	require.NoError(t, err)
