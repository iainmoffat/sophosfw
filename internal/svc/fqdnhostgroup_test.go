...
// TestFQDNHostGroup_Create_DoesNotMutateCallerBody is a regression
// guard for a parallel-preflight race. Phase 14 fan-out runs N
// preflight goroutines in parallel against the same body map. The
// svc-layer _diffHash strip used to call delete(body, …) on the
// caller's map, which under N parallel goroutines triggers Go's
// "concurrent map writes" runtime panic. The fix clones the body
// before the strip; this test asserts the caller's map is unchanged
// after Create.
func TestFQDNHostGroup_Create_DoesNotMutateCallerBody(t *testing.T) {
	s, _, _ := newFQDNHostGroupSvc(t, nil)
	body := map[string]any{
		"Name":      "FG1",
		"IPFamily":  "IPv4",
		"_diffHash": "abc",
	...
	_, err := s.Create(context.Background(), "home", "FG1", body, true /* dryRun */)
	require.NoError(t, err)
