...
// fakeFqdnHostClient is the test double for FQDNHostSvc. It answers
...
// shape.
type fakeFqdnHostClient struct {
	body map[string]any
	sent [][]byte
}

func (f *fakeFqdnHostClient) Do(_ context.Context, env sophos.Envelope) (*sophos.Response, error) {
...
func (f *fakeFqdnHostClient) DoRaw(_ context.Context, raw []byte) (*sophos.Response, error) {
	f.sent = append(f.sent, raw)
	return &sophos.Response{LoginOK: true}, nil
}

// newFQDNHostSvc constructs an FQDNHostSvc backed by a fake client
// primed with `body` (the live FQDNHost record returned to Get calls).
// Returns the svc, the fake client (for sent-envelope assertions), and
// the audit dir for log assertions.
func newFQDNHostSvc(t *testing.T, body map[string]any) (*FQDNHostSvc, *fakeFqdnHostClient, string) {
	t.Helper()
	...
	fc := &fakeFqdnHostClient{body: body}
	s := &FQDNHostSvc{
		Inner: &ObjectSvc{
...
func TestFQDNHost_Create_RejectsMissingRequiredField(t *testing.T) {
	s, fc, _ := newFQDNHostSvc(t, nil)
...
func TestFQDNHost_Create_RejectsReadOnlyProfile(t *testing.T) {
	s, fc, _ := newFQDNHostSvc(t, nil)
...
func TestFQDNHost_Create_DryRun_EmitsPreview(t *testing.T) {
	s, fc, _ := newFQDNHostSvc(t, nil)
	body := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	...
	require.Equal(t, "FQDNHost", out.ObjectType)
	require.NotNil(t, out.Preview)
	require.True(t, out.Preview.Mutating)
	require.Empty(t, fc.sent, "dry-run must not send")
}

func TestFQDNHost_Create_Apply_SendsAddEnvelope(t *testing.T) {
	body := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	s, fc, _ := newFQDNHostSvc(t, body)
...
func TestFQDNHost_Update_RejectsMissingExpectedDiffHash(t *testing.T) {
	body := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	s, fc, _ := newFQDNHostSvc(t, body)
...
func TestFQDNHost_Update_RejectsHashMismatch(t *testing.T) {
	body := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	s, fc, _ := newFQDNHostSvc(t, body)
...
func TestFQDNHost_Update_DryRun_EmitsPreview(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	hash, _ := DiffHash(live)
	s, fc, _ := newFQDNHostSvc(t, live)
...
func TestFQDNHost_Update_Apply_SendsUpdateEnvelope(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	hash, _ := DiffHash(live)
	s, fc, _ := newFQDNHostSvc(t, live)
...
func TestFQDNHost_Delete_RejectsMissingExpectedDiffHash(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	s, fc, _ := newFQDNHostSvc(t, live)
...
func TestFQDNHost_Delete_RejectsHashMismatch(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	s, fc, _ := newFQDNHostSvc(t, live)
...
func TestFQDNHost_Delete_Apply_SendsRemoveEnvelope(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	hash, _ := DiffHash(live)
	s, fc, auditDir := newFQDNHostSvc(t, live)
	...
	require.NoError(t, err)
	require.Contains(t, string(logBody), `"operation":"fqdn_host_delete"`)
	require.Contains(t, string(logBody), `"objectName":"F1"`)
}

func TestFQDNHost_Delete_OnMissing_ReturnsNotFound(t *testing.T) {
	s, fc, _ := newFQDNHostSvc(t, nil) // nil body → Get returns not_found
...
// TestFQDNHost_Update_OnStubRecord_ReturnsNotFoundWithName ensures the
...
// `FQDNHost "": not_found`.
func TestFQDNHost_Update_OnStubRecord_ReturnsNotFoundWithName(t *testing.T) {
...
// TestFQDNHost_Update_StripsInjectedDiffHashFromBody verifies that
...
func TestFQDNHost_Update_StripsInjectedDiffHashFromBody(t *testing.T) {
	live := map[string]any{"Name": "F1", "FQDN": "example.com", "IPFamily": "IPv4"}
	hash, _ := DiffHash(live)
	s, fc, _ := newFQDNHostSvc(t, live)
...
// TestFQDNHost_Create_DoesNotMutateCallerBody is a regression guard
...
func TestFQDNHost_Create_DoesNotMutateCallerBody(t *testing.T) {
	s, _, _ := newFQDNHostSvc(t, nil)
	body := map[string]any{
		"Name":      "F1",
		"FQDN":      "example.com",
		"IPFamily":  "IPv4",
	...
	_, err := s.Create(context.Background(), "home", "F1", body, true /* dryRun */)
	require.NoError(t, err)
