// Package svc — FQDNHost mutating surface (Phase 12, Task 7).
//
// Body-as-map create/update/delete for FQDNHost objects. Mechanical
...
// FQDNHostSvc owns the create/update/delete surface for FQDNHost.
...
func (s *FQDNHostSvc) now() time.Time {
	if s.Now != nil {
		return s.Now()
	}
	return time.Now().UTC()
...
// requiredFQDNHostFields are the keys a create/update body must
...
var requiredFQDNHostFields = []string{"Name", "FQDN", "IPFamily"}

// Create adds a new FQDNHost. The body must include all keys listed
// in requiredFQDNHostFields. dryRun=true returns a Preview without
// sending the envelope.
func (s *FQDNHostSvc) Create(ctx context.Context, profileName, name string, body map[string]any, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "create", "", false, dryRun)
}

// Update replaces an existing FQDNHost body. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *FQDNHostSvc) Update(ctx context.Context, profileName, name string, body map[string]any, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "update", expectedHash, ignoreHash, dryRun)
}

// Delete removes a FQDNHost by name. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *FQDNHostSvc) Delete(ctx context.Context, profileName, name, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
...
func (s *FQDNHostSvc) mutate(ctx context.Context, profileName, name string, body map[string]any, op, expectedHash string, ignoreHash, dryRun bool) (out *ObjectMutationResult, err error) {
	...
	catEntry, ok := s.Inner.Catalog.Resolve("FQDNHost")
	if !ok || !catEntry.Mutable {
		return nil, fmt.Errorf("%w: FQDNHost is not flagged mutable in the catalog", sophos.ErrInvalidRequest)
	}
	...
	if op != "delete" {
		for _, k := range requiredFQDNHostFields {
			v, present := body[k]
			if !present {
	...
	switch op {
	case "create":
		inner, merr := marshalObjectBody("FQDNHost", body)
		if merr != nil {
			return nil, merr
		}
	...
	case "delete":
		...
		inner.WriteString("<FQDNHost><Name>")
		if perr = xml.EscapeText(&inner, []byte(name)); perr != nil {
			return nil, perr
		}
		inner.WriteString("</Name></FQDNHost>")
...
// fetchLive loads the live FQDNHost body as a map[string]any. The
...
func (s *FQDNHostSvc) fetchLive(ctx context.Context, profileName, name string) (map[string]any, error) {
	obj, err := s.Inner.Get(ctx, profileName, "FQDNHost", name)
	if err != nil {
		return nil, err
	}
	m, ok := obj.Data.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("FQDNHostSvc: catalog returned non-map FQDNHost payload: %T", obj.Data)
	}
	if liveName, _ := m["Name"].(string); liveName == "" {
		...
		return nil, fmt.Errorf("FQDNHost %q: %w", name, sophos.ErrNotFound)
	}
	return m, nil
}
