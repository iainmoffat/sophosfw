...
// TestServiceGroup_Create_DoesNotMutateCallerBody is a regression
...
// after Create.
func TestServiceGroup_Create_DoesNotMutateCallerBody(t *testing.T) {
	s, _, _ := newServiceGroupSvc(t, nil)
	body := map[string]any{
		"Name":      "SG1",
		"_diffHash": "abc",
	}
	...
	_, err := s.Create(context.Background(), "home", "SG1", body, true /* dryRun */)
	require.NoError(t, err)
