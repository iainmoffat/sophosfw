// Package svc — ServiceGroup mutating surface (Phase 12, Task 11).
//
...
// ServiceGroupSvc owns the create/update/delete surface for ServiceGroup.
...
func (s *ServiceGroupSvc) now() time.Time {
	if s.Now != nil {
		return s.Now()
	}
	return time.Now().UTC()
...
// requiredServiceGroupFields are the keys a create/update body must
...
var requiredServiceGroupFields = []string{"Name"}

// Create adds a new ServiceGroup. The body must include all keys listed
// in requiredServiceGroupFields. dryRun=true returns a Preview without
// sending the envelope.
func (s *ServiceGroupSvc) Create(ctx context.Context, profileName, name string, body map[string]any, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "create", "", false, dryRun)
}

// Update replaces an existing ServiceGroup body. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *ServiceGroupSvc) Update(ctx context.Context, profileName, name string, body map[string]any, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "update", expectedHash, ignoreHash, dryRun)
}

// Delete removes a ServiceGroup by name. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *ServiceGroupSvc) Delete(ctx context.Context, profileName, name, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
...
func (s *ServiceGroupSvc) mutate(ctx context.Context, profileName, name string, body map[string]any, op, expectedHash string, ignoreHash, dryRun bool) (out *ObjectMutationResult, err error) {
	...
	catEntry, ok := s.Inner.Catalog.Resolve("ServiceGroup")
	if !ok || !catEntry.Mutable {
		return nil, fmt.Errorf("%w: ServiceGroup is not flagged mutable in the catalog", sophos.ErrInvalidRequest)
	}
	...
	if op != "delete" {
		for _, k := range requiredServiceGroupFields {
			v, present := body[k]
			if !present {
	...
	switch op {
	case "create":
		inner, merr := marshalObjectBody("ServiceGroup", body)
		if merr != nil {
			return nil, merr
		}
	...
	case "delete":
		...
		inner.WriteString("<ServiceGroup><Name>")
		if perr = xml.EscapeText(&inner, []byte(name)); perr != nil {
			return nil, perr
		}
		inner.WriteString("</Name></ServiceGroup>")
...
// fetchLive loads the live ServiceGroup body as a map[string]any. The
...
func (s *ServiceGroupSvc) fetchLive(ctx context.Context, profileName, name string) (map[string]any, error) {
	obj, err := s.Inner.Get(ctx, profileName, "ServiceGroup", name)
	if err != nil {
		return nil, err
	}
	m, ok := obj.Data.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("ServiceGroupSvc: catalog returned non-map ServiceGroup payload: %T", obj.Data)
	}
	if liveName, _ := m["Name"].(string); liveName == "" {
		...
		return nil, fmt.Errorf("ServiceGroup %q: %w", name, sophos.ErrNotFound)
	}
	return m, nil
}
