// Package svc — FQDNHostGroup mutating surface (Phase 12, Task 8).
//
// Body-as-map create/update/delete for FQDNHostGroup objects. Mechanical
// mirror of IPHostGroupSvc; see internal/svc/iphostgroup.go for the
...
// FQDNHostGroupSvc owns the create/update/delete surface for FQDNHostGroup.
...
func (s *FQDNHostGroupSvc) now() time.Time {
	if s.Now != nil {
		return s.Now()
	}
	return time.Now().UTC()
...
// requiredFQDNHostGroupFields are the keys a create/update body must
...
var requiredFQDNHostGroupFields = []string{"Name", "IPFamily"}

// Create adds a new FQDNHostGroup. The body must include all keys listed
// in requiredFQDNHostGroupFields. dryRun=true returns a Preview without
// sending the envelope.
func (s *FQDNHostGroupSvc) Create(ctx context.Context, profileName, name string, body map[string]any, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "create", "", false, dryRun)
}

// Update replaces an existing FQDNHostGroup body. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *FQDNHostGroupSvc) Update(ctx context.Context, profileName, name string, body map[string]any, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
	return s.mutate(ctx, profileName, name, body, "update", expectedHash, ignoreHash, dryRun)
}

// Delete removes a FQDNHostGroup by name. expectedHash is required
// (unless ignoreHash is true) and must match the live record's
// DiffHash; otherwise ErrDiffHashMismatch is returned.
func (s *FQDNHostGroupSvc) Delete(ctx context.Context, profileName, name, expectedHash string, ignoreHash, dryRun bool) (*ObjectMutationResult, error) {
...
func (s *FQDNHostGroupSvc) mutate(ctx context.Context, profileName, name string, body map[string]any, op, expectedHash string, ignoreHash, dryRun bool) (out *ObjectMutationResult, err error) {
	...
	catEntry, ok := s.Inner.Catalog.Resolve("FQDNHostGroup")
	if !ok || !catEntry.Mutable {
		return nil, fmt.Errorf("%w: FQDNHostGroup is not flagged mutable in the catalog", sophos.ErrInvalidRequest)
	}
	...
	if op != "delete" {
		for _, k := range requiredFQDNHostGroupFields {
			v, present := body[k]
			if !present {
	...
	switch op {
	case "create":
		inner, merr := marshalObjectBody("FQDNHostGroup", body)
		if merr != nil {
			return nil, merr
		}
	...
	case "delete":
		...
		inner.WriteString("<FQDNHostGroup><Name>")
		if perr = xml.EscapeText(&inner, []byte(name)); perr != nil {
			return nil, perr
		}
		inner.WriteString("</Name></FQDNHostGroup>")
...
// fetchLive loads the live FQDNHostGroup body as a map[string]any. The
...
func (s *FQDNHostGroupSvc) fetchLive(ctx context.Context, profileName, name string) (map[string]any, error) {
	obj, err := s.Inner.Get(ctx, profileName, "FQDNHostGroup", name)
	if err != nil {
		return nil, err
	}
	m, ok := obj.Data.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("FQDNHostGroupSvc: catalog returned non-map FQDNHostGroup payload: %T", obj.Data)
	}
	if liveName, _ := m["Name"].(string); liveName == "" {
		...
		return nil, fmt.Errorf("FQDNHostGroup %q: %w", name, sophos.ErrNotFound)
	}
	return m, nil
}
